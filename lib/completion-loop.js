...
export async function waitForCompletion(cascadeId, options = {}) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        try {
            ...
            // ── Null/Unknown status after steps = cascade completed and fell off active list ──
            // getAllStatuses() stops returning a cascade once it's no longer active.
            // If we had steps before and status is now gone, treat as completed.
            if ((status === 'UNKNOWN' || !raw.status) && peakStepCount > 0) {
                log(`[${cascadeId.substring(0, 8)}] ✓ Status gone (raw.status=${raw.status}) after ${peakStepCount} steps — treating as completed`);
