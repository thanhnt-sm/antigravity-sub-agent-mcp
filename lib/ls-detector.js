...
export function detectPorts(pid) {
    return new Promise((resolve) => {
        // @BR-SEC-01 — Validate PID is integer before shell interpolation
        const safePid = parseInt(pid, 10);
        if (isNaN(safePid) || safePid <= 0) { resolve([]); return; }
        let cmd;
        if (platform === 'win32') {
            cmd = `netstat -ano`;
        } else {
            cmd = `lsof -iTCP -sTCP:LISTEN -P -n -p ${safePid} 2>/dev/null`;
        }
...
async function detectFromParentProcess() {
    ...
    // @BR-SEC-01 — Validate PPID is integer before shell interpolation
    const safePpid = parseInt(ppid, 10);
    if (isNaN(safePpid) || safePpid <= 0) return null;

    log(`Checking parent process (PPID=${safePpid})...`);

    // Read parent's command line
    return new Promise((resolve) => {
        const cmd = platform === 'win32'
            ? `wmic process where ProcessId=${safePpid} get CommandLine /format:list 2>nul`
            : `ps -p ${safePpid} -o command= 2>/dev/null`;
